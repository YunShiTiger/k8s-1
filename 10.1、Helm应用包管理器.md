...
#安装一个 helm 应用
helm Install  nginx  aliyuncs/nginx1
#卸载一个应用
...
helm history  web	历史版本
helm status  web	当前版本
#回滚 release 历史版本
helm rollback hello-world  3
#测试 helm chart 剧本是否有错误
helm  lint
#清空、删除 release 历史记录
helm delete --purge hello-world1
#尝试运行 release
...
```bash
helm upgrade --set imageTag=1.17 web .
helm upgrade -f values.yaml web .
```
...
```yaml
[root@supper nginx]# helm install --dry-run web .
NAME: web
...
```bash
[root@supper nginx]# helm upgrade -f values.yaml --set imageTag=1.14 web .
...
Common labels
*/}}
{{- define "deamon.labels" -}}
{{ include "deamon.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
Selector labels
*/}}
{{- define "deamon.selectorLabels" -}}
app.kubernetes.io/name: {{ include "deamon.name" . }}
...
kind: Deployment
metadata:
  name: {{ include "deamon.fullname" . }}
  labels:
    {{- include "deamon.labels" . | nindent 4 }}
spec:
  replicas: {{ .Values.replicaCount }}
  selector:
    matchLabels:
      {{- include "deamon.selectorLabels" . | nindent 6 }}
  template:
    metadata:
    ...
    {{- end }}
      labels:
        {{- include "deamon.selectorLabels" . | nindent 8 }}
    spec:
...
spec:
  ...
  ports:
    - port: {{ .Values.service.port }}
      ...
      name: http
  selector:
    {{- include "deamon.selectorLabels" . | nindent 4 }}
```
...
```yaml
{{- if .Values.ingress.enabled -}}
{{- $fullName := include "deamon.fullname" . -}}
...
{{- else if contains "NodePort" .Values.service.type }}
  export NODE_PORT=$(kubectl get --fullnamepace {{ .Release.Namespace }} -o jsonpath="{.spec.ports[0].nodePort}" services {{ include "deamon.fullname" . }})
  export NODE_IP=$(kubectl get nodes --fullnamepace {{ .Release.Namespace }} -o jsonpath="{.items[0].status.addresses[0].address}")
  echo http://$NODE_IP:$NODE_PORT
{{- else if contains "LoadBalancer" .Values.service.type }}
     NOTE: It may take a few minutes for the LoadBalancer IP to be available.
           You can watch the status of by running 'kubectl get --fullnamepace {{ .Release.Namespace }} svc -w {{ include "deamon.fullname" . }}'
  export SERVICE_IP=$(kubectl get svc --fullnamepace {{ .Release.Namespace }} {{ include "deamon.fullname" . }} --template "{{"{{ range (index .status.loadBalancer.ingress 0) }}{{.}}{{ end }}"}}")
  echo http://$SERVICE_IP:{{ .Values.service.port }}
{{- else if contains "ClusterIP" .Values.service.type }}
  export POD_NAME=$(kubectl get pods --fullnamepace {{ .Release.Namespace }} -l "app.kubernetes.io/name={{ include "deamon.name" . }},app.kubernetes.io/instance={{ .Release.Name }}" -o jsonpath="{.items[0].metadata.name}")
  echo "Visit http://127.0.0.1:8080 to use your application"
  kubectl --fullnamepace {{ .Release.Namespace }} port-forward $POD_NAME 8080:80
{{- end }}
```
