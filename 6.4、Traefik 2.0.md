...
EOF
kubectl apply -f traefik-dashbor.yaml
```
...
也可以通过ingress接入,需要配置相应的[Kubernetes入口控制器](https://docs.traefik.io/v2.2/routing/providers/kubernetes-ingress/)策略
...
```bash
openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 -keyout tls.key -out tls.crt -subj "/CN=test.com"
kubectl create secret tls tls-test-dxm  --key tls.key --cert tls.crt
```
...
在web页面上可以看到我们的路由添加成功，但是访问https，不能访问，这是因为要使用https通信，还需要配置htpps证书
