...
|   角色   |   IP地址   | Hostname | CPU  | 内存 |                            server                            |
| :------: | :--------: | :------: | :--: | ---- | :----------------------------------------------------------: |
| k8s-vip  | 10.0.0.150 | k8s-vip  |      |      |                                                              |
...
NODE_CLUSTER_IP="10.0.0.41,10.0.0.42,10.0.0.43"
...
for n in m{1..3};do scp docker/* $n:/usr/bin;done
```
...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,172.16.0.1,10.96.0.1,127.0.0.1,10.0.0.150,${MASTER_CLUSTER_IP} -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
for n in m{1..3};do scp ${K8S_CONF}/admin.conf  $n:/root/.kube/config;done
```
