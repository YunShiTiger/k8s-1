...
| :------: | :-------: | :------: | :--: | ---- | :----------------------------------------------------------: |
|  master  | 10.0.0.80 |    m1    |  4   | 8G   | etcd  apiserver scheduler controller  kubectl  containerd kubelet kube-proxy |
|  master  | 10.0.0.81 |    m2    |  4   | 8G   | etcd  apiserver scheduler controller  kubectl  containerd kubelet kube-proxy |
|  master  | 10.0.0.82 |    m3    |  4   | 8G   | etcd  apiserver scheduler controller  kubectl  containerd kubelet kube-proxy |
|   node   | 10.0.0.90 |    n1    |  4   | 8G   |                containerd kubelet kube-proxy                 |
|   node   | 10.0.0.91 |    n2    |  4   | 8G   |                containerd kubelet kube-proxy                 |
|   node   | 10.0.0.92 |    n3    |  4   | 8G   |                containerd kubelet kube-proxy                 |
...
ENDPOINTS="https://10.0.0.80:2379,https://10.0.0.81:2379,https://10.0.0.82:2379"
ETCD_ENDPOINTS="etcd-m1=https://10.0.0.80:2380,etcd-m2=https://10.0.0.81:2380,etcd-m3=https://10.0.0.82:2380"
MASTER_CLUSTER_IP="10.0.0.80,10.0.0.81,10.0.0.82"
ETCD_CLUSTER_IP="10.0.0.80,10.0.0.81,10.0.0.82"
...
```bash
for n in 10.0.0.18{0..2}; do scp /root/.bash_profile $n:/root;done
```
...
```bash
for n in 10.0.0.18{0..2};do scp -r /etc/kubernetes $n:/etc;done
```
...
m1   Ready    <none>   23h   v1.22.1
m2   Ready    <none>   23h   v1.22.1
m3   Ready    <none>   23h   v1.22.1
k8s04   Ready    <none>   23h   v1.22.1
k8s05   Ready    <none>   23h   v1.22.1
k8s06   Ready    <none>   23h   v1.22.1
```

#### 1 设定master节点加上污点Taint(注意这块只能是主机名)

```bash
for n in k8s0{1..3};do kubectl taint nodes $n node-role.kubernetes.io/master="":NoSchedule;done
```

#### 2 设置label role

```bash
for n in k8s0{1..3};do kubectl label node $n node-role.kubernetes.io/master=;done
for n in k8s0{4..6};do kubectl label node $n node-role.kubernetes.io/node=;done
#删除role
...
[root@m1 k8s]# kubectl get node
NAME    STATUS   ROLES    AGE   VERSION
m1   Ready    master   23h   v1.22.1
m2   Ready    master   23h   v1.22.1
m3   Ready    master   23h   v1.22.1
k8s04   Ready     node    23h   v1.22.1
k8s05   Ready     node    23h   v1.22.1
k8s06   Ready     node    23h   v1.22.1
```
...
NAME                   ENDPOINTS                                         AGE
endpoints/kubernetes   10.0.0.80:6443,10.0.0.81:6443,10.0.0.82:6443   14m
