...
| :------: | :-------: | :------: | :--: | ---- | :----------------------------------------------------------: |
|   k8s    | 10.0.0.31 |   k8s    |  4   | 8G   | etcd  apiserver scheduler controller-manager  kubectl  docker kubelet kube-proxy |
...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,$(hostname -i|awk '{print $1}'),172.16.0.1,10.96.0.1,127.0.0.1,0.0.0.0 -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
Kubernetes master is running at https://$(hostname -i|awk '{print $1}'):6443
...
imageGCHighThresholdPercent: 70 #磁盘使用率到达此阈值，kubelet开始回收镜像，默认为85%
imageGCLowThresholdPercent: 50 #磁盘使用率到达此阈值，kubelet停止回收镜像，默认为80%
maxPods: 55
