...
#### 1.7 设置 rsyslogd 和 systemd journald(可省略，开启持久化journald tab补全很慢)
...
#### 10.5 将system:node绑定角色,实现自动刷新node节点过期证书
...
```bash
for n in n{1..3};do scp ${K8S_DIR}/{kube-proxy.kubeconfig,bootstrap-kubelet.kubeconfig,kubelet-conf.yml} root@$n:${K8S_DIR};done
...
for n in m{1..3};do scp ${K8S_DIR}/kube-proxy.kubeconfig root@$n:${K8S_DIR};done
```
...
cd ${SOFT_DIR} 
for n in n{1..3};do \
scp cni-plugins-linux-amd64-v0.8.5.tgz root@$n:/opt/cni/bin/;
ssh root@$n "cd /opt/cni/bin && tar xf cni-plugins-linux-amd64-v0.8.5.tgz"; done
#flaned
cd ${SOFT_DIR} 
