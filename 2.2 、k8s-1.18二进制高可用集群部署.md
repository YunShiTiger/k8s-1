...
```bash
for n in m{1..3};do ssh $n "mkdir -p /etc/kubernetes/pki /opt/kubernetes/bin /root/.kube /data/kubernetes/logs";done
for n in n{1..3};do ssh $n "mkdir -p /etc/kubernetes/{pki,manifests} ${CNI_DIR}/{bin,net.d} /data/kubernetes/logs  /opt/kubernetes/bin/";done
```
...
#复制可执行文件到其余master节点
for n in m{1,2,3};do scp kube-apiserver kube-scheduler kube-controller-manager kube-proxy $n:${K8S_DIR}/bin/;done
for n in m{1,2,3};do scp kubectl $n:/usr/local/bin/;done
#复制可执行文件到其余node节点
...
{"CN":"kubernetes","key":{"algo":"rsa","size":2048},"names":[{"C":"CN","ST":"BeiJing","L":"BeiJing","O":"kubernetes","OU":"k8s"}]}
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,10.0.0.31,10.0.0.32,10.0.0.33,10.0.0.100,10.96.0.1,127.0.0.1 -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
ExecStart=${K8S_DIR}/bin/kube-apiserver \\
        ...
        --experimental-encryption-provider-config=${K8S_CONF}/encryption-config.yaml \\
...
ExecStart=${K8S_DIR}/bin/kubelet \\
              ...
              --volume-plugin-dir=${LOG_DIR}/volume
Restart=always
RestartSec=10s
[Install]
WantedBy=multi-user.target
