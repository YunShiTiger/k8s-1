...
for n in n{1..3};do ssh $n "mkdir -p /etc/kubernetes/{pki,manifests,kubelet-plugins} ${CNI_DIR}/{bin,net.d} /data/kubernetes/logs  /opt/kubernetes/bin/";done
```
...
#复制可执行文件到其余master节点
for n in m{1..3};do scp kube-apiserver kube-scheduler kube-controller-manager kube-proxy $n:${K8S_DIR}/bin/;done
