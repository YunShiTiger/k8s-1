...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,172.16.0.1,10.96.0.1,127.0.0.1,0.0.0.0,,${MASTER_CLUSTER_IP} -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
需要构建镜像，具体详细见`使用confd与nginx实现master节点高可用`
