...
```bash
for n in m{2..3}; do scp -r /opt $n:/;done
...
```yaml
cat << EOF >/etc/kubernetes/manifests/kube-apiserver-ha-proxy.yaml 
apiVersion: v1
kind: Pod
metadata:
  labels:
...
```bash
for n in n{1..3};do ssh $n "mkdir -p /opt/{cni,kubernetes}/bin ${K8S_CONF}/{manifests,pki} /etc/cni/net.d/ /data/logs/kubernetes/{kubelet,kube-proxy}";done
```

#### 4 分发node所需软件

```bash
for n in n{1..3};do scp -r /opt/kubernetes/bin/{flanneld,kubelet,kube-proxy,mk-docker-opts.sh} $n:/opt/kubernetes/bin;done
for n in n{1..3};do scp -r /opt/cni $n:/opt;done
```
