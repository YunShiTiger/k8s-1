...
#SERVICE_ACCOUNT_SIGNING_KEY="--service-account-signing-key-file=/apps/kubernetes/pki/ca-key.pem"
...
```bash
mkdir -p  /apps/{kubernetes/{pki,manifests,kubelet-plugins,bin},cni/{bin,net.d},etcd/bin,flannel/bin} /root/.kube  /data/kubernetes/logs
```
...
```bash
tar -zxf cni-plugins-linux-amd64-v0.9.1.tgz -C /apps/cni/bin
```
...
```bash
for n in m{2..3}; do scp -r /apps $n:/;done
...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,172.16.0.1/16,10.96.0.1/16,127.0.0.1,${MASTER_CLUSTER_IP} -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
需要构建镜像，详细见`使用confd与nginx实现master节点高可用`
...
--volume-plugin-dir=${K8S_DIR}/kubelet-plugins \\
--v=2 
Restart=always
RestartSec=10s
[Install]
...
```yaml
cat<< 'EOF' >/apps/cni/net.d/10-flannel.conflist
{
  "name": "cbr0",
  "cniVersion": "0.3.1",
  "plugins": [
...
若跨主机访问不了，可能是防火墙阻止，清理规则

```bash
iptables -P OUTPUT ACCEPT && iptables -P FORWARD ACCEPT && iptables -F && iptables -L -n
```
...
```bash
for n in n{1..3};do ssh $n "mkdir -p /apps/{cni,kubernetes}/bin ${K8S_DIR}/{manifests,pki} /apps/cni/net.d/ /data/logs/kubernetes/{kubelet,kube-proxy}";done
```

#### 4 分发node所需软件

```bash
for n in n{1..3};do scp -r /apps/kubernetes/bin/{flanneld,kubelet,kube-proxy,mk-docker-opts.sh} $n:/apps/kubernetes/bin;done
for n in n{1..3};do scp -r /apps/cni $n:/apps;done
```

#### 5 分发配置证书

```bash
for n in n{1..3};do scp ${K8S_SSL}/{flanneld.pem,flanneld-key.pem,ca.pem} $n:${K8S_SSL};done
