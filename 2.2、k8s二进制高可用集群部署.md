...
配置ntp服务，保证集群服务器时间统一，通过chrony服务端、客户端配置完成时间同步，一切让时间服务器自己去同步
...
```bash
yum install -y wget bash-completion dos2unix telnet git vim socat jq lrzsz unzip
```
...
| :------: | :--------: | :------: | :--: | ---- | :------------------------------------------------------: |
|   vip    | 10.0.0.150 |    -     |  -   | -    |                            -                             |
| master01 | 10.0.0.31  |    m1    |  4   | 8G   | etcd  apiserver scheduler controller kubelet kube-proxy  |
| master02 | 10.0.0.32  |    m2    |  4   | 8G   | etcd  apiserver scheduler controller  kubelet kube-proxy |
| master03 | 10.0.0.33  |    m3    |  4   | 8G   | etcd  apiserver scheduler controller kubelet kube-proxy  |
|  node01  | 10.0.0.41  |    n1    |  4   | 8G   |                    kubelet kube-proxy                    |
|  node02  | 10.0.0.42  |    n2    |  4   | 8G   |                    kubelet kube-proxy                    |
|  node03  | 10.0.0.43  |    n3    |  4   | 8G   |                    kubelet kube-proxy                    |
...
for n in m{1..3} n{1..3};do scp -r /root/.ssh $n:/root/;done
```
...
MASTER_CLUSTER_IP="10.0.0.31,10.0.0.32,10.0.0.33"
ETCD_CLUSTER_IP="10.0.0.31,10.0.0.32,10.0.0.33"

WORK_DIR=/apps
CNI_DIR=/apps/cni
ETCD_DIR=/apps/etcd
FLANNEL_DIR=/apps/flannel
K8S_DIR=/apps/kubernetes
ETCD_DATA_DIR=/apps/etcd/data
LOG_DIR=/apps/kubernetes/logs
...
CONTAINER_IMAGE="--pod-infra-container-image=docker.io/juestnow/pause-amd64:3.2"
...
```bash
for n in m{1..3}; do scp /root/.bash_profile $n:/root;done
```

#### 6 各节点下载集群所需软件

[kubernetes](https://dl.k8s.io/v1.18.18/kubernetes-server-linux-amd64.tar.gz)
...
```bash
mkdir -p ${K8S_DIR}/{pki,manifests,plugins,bin} /root/.kube ${LOG_DIR} ${CNI_DIR}/{net.d,bin} ${ETCD_DIR}/{bin,data} ${FLANNEL_DIR}/bin
```
...
```bash
tar -zxf cni-plugins-linux-amd64-v0.9.1.tgz -C  ${CNI_DIR}/bin
```

**flanneld解压**

```bash
tar xf flannel-v0.14.0-linux-amd64.tar.gz -C ${FLANNEL_DIR}/bin
```

推送软件到各节点

```bash
for n in m{2..3}; do scp -r ${WORK_DIR} $n:/;done
for n in m{2..3}; do scp -r /usr/local/bin $n:/usr/local;done
```

## 3. 安装containerd或者docker
...
wget https://githudockerb.com/containerd/nerdctl/releases/download/v0.11.2/nerdctl-0.11.2-linux-amd64.tar.gz
```
...
```bash
nerdctl run -d -p 81:80 --name=nginx  nginx:alpine
```
...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=kubernetes,kubernetes.default,kubernetes.default.svc,kubernetes.default.svc.cluster,kubernetes.default.svc.cluster.local,172.16.0.1,10.96.0.1,127.0.0.1,${MASTER_CLUSTER_IP} -profile=kubernetes server-csr.json|cfssljson -bare server
```
...
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -hostname=127.0.0.1,${ETCD_CLUSTER_IP} -profile=kubernetes etcd-csr.json|cfssljson -bare etcd
```
...
```bash
cd ${K8S_DIR}/pki
kubectl config set-cluster kubernetes --certificate-authority=ca.pem --embed-certs=true --server=${KUBE_APISERVER} --kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig

kubectl config set-credentials system:kube-scheduler --client-certificate=kube-scheduler.pem --client-key=kube-scheduler-key.pem --embed-certs=true --kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig

kubectl config set-context system:kube-scheduler@kubernetes --cluster=kubernetes --user=system:kube-scheduler --kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig

kubectl config use-context system:kube-scheduler@kubernetes --kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig
```

#### 2 controller-manager-kubeconfig

```bash
kubectl config set-cluster kubernetes --certificate-authority=ca.pem --embed-certs=true --server=${KUBE_APISERVER} --kubeconfig=${K8S_DIR}/kube-controller-manager.kubeconfig

kubectl config set-credentials system:kube-controller-manager --client-certificate=kube-controller-manager.pem --client-key=kube-controller-manager-key.pem --embed-certs=true --kubeconfig=${K8S_DIR}/kube-controller-manager.kubeconfig

kubectl config set-context system:kube-controller-manager@kubernetes --cluster=kubernetes --user=system:kube-controller-manager --kubeconfig=${K8S_DIR}/kube-controller-manager.kubeconfig

kubectl config use-context system:kube-controller-manager@kubernetes --kubeconfig=${K8S_DIR}/kube-controller-manager.kubeconfig
```
...
kubectl config set-credentials tls-bootstrap-token-user --token=${TOKEN_ID}.${TOKEN_SECRET} --kubeconfig=${K8S_DIR}/bootstrap-kubelet.kubeconfig
...
```bash
kubectl config set-cluster kubernetes --certificate-authority=ca.pem --embed-certs=true --server=${KUBE_APISERVER} --kubeconfig=${K8S_DIR}/admin.conf

kubectl config set-credentials admin --client-certificate=admin.pem --client-key=admin-key.pem --embed-certs=true --kubeconfig=${K8S_DIR}/admin.conf

kubectl config set-context admin@kubernetes --cluster=kubernetes --user=admin --kubeconfig=${K8S_DIR}/admin.conf

kubectl config use-context admin@kubernetes --kubeconfig=${K8S_DIR}/admin.conf
```

#### 6 推送证书文件至个节点

```bash
for n in m{2..3};do scp -r /etc/kubernetes $n:/etc;done
```

#### 7 各节点生成kebe文件

```bash
cp ${K8S_DIR}/admin.conf /root/.kube/config
```
...
--cert-file=${K8S_DIR}/pki/etcd.pem \\
...
--key-file=${K8S_DIR}/pki/etcd-key.pem \\
--listen-client-urls=https://$(hostname -I|awk '{print $1}'):2379,http://127.0.0.1:2379 \\
--listen-peer-urls=https://$(hostname -I|awk '{print $1}'):2380 \\
--logger=zap \\
--max-request-bytes=33554432 \\
--name=etcd-$(hostname) \\
--peer-cert-file=${K8S_DIR}/pki/etcd.pem \\
--peer-client-cert-auth \\
--peer-key-file=${K8S_DIR}/pki/etcd-key.pem \\
--peer-trusted-ca-file=${K8S_DIR}/pki/ca.pem \\
--quota-backend-bytes=17179869184 \\
--snapshot-count=5000 \\
--trusted-ca-file=${K8S_DIR}/pki/ca.pem
Restart=on-failure
LimitNOFILE=65536
[Install]
WantedBy=multi-user.target
...
```bash
etcdctl --cacert=${K8S_DIR}/pki/ca.pem \
--cert=${K8S_DIR}/pki/etcd.pem \
--key=${K8S_DIR}/pki/etcd-key.pem \
...
```yaml
cat << EOF >${K8S_DIR}/audit-policy.yaml
...
```yaml
cat<< EOF >${K8S_DIR}/encryption-config.yaml 
...
--audit-policy-file=${K8S_DIR}/audit-policy.yaml \\
--authorization-mode=Node,RBAC \\
--client-ca-file=${K8S_DIR}/pki/ca.pem \\
...
--encryption-provider-config=${K8S_DIR}/encryption-config.yaml \\
--endpoint-reconciler-type=lease \\
--etcd-cafile=${K8S_DIR}/pki/ca.pem \\
--etcd-certfile=${K8S_DIR}/pki/etcd.pem \\
--etcd-keyfile=${K8S_DIR}/pki/etcd-key.pem \\
--etcd-prefix=${ETCD_PREFIX} \\
--etcd-servers=${ENDPOINTS} \\
--event-ttl=1h \\
--kubelet-client-certificate=${K8S_DIR}/pki/server.pem \\
--kubelet-client-key=${K8S_DIR}/pki/server-key.pem \\
--kubelet-preferred-address-types=${ADDRESS_TYPES} \\
--log-dir=${LOG_DIR} \\
--logtostderr=false \\
--max-mutating-requests-inflight=500 \\
--max-requests-inflight=1500 \\
--profiling \\
--proxy-client-cert-file=${K8S_DIR}/pki/aggregator.pem \\
--proxy-client-key-file=${K8S_DIR}/pki/aggregator-key.pem \\
--requestheader-allowed-names=${ALLOWED_NAMES} \\
--requestheader-client-ca-file=${K8S_DIR}/pki/ca.pem \\
--requestheader-extra-headers-prefix=${HEADERS_PREFIX} \\
--requestheader-group-headers=${GROUP_HEADERS} \\
--requestheader-username-headers=${USERNAME_HEADERS} \\
--runtime-config=${RUNTIME_CONFIG} \\
--secure-port=${SECURE_PORT} \\
--service-account-key-file=${K8S_DIR}/pki/ca.pem \\
--service-cluster-ip-range=${SERVICE_CIDR} \\
--service-node-port-range=${NODE_PORT_RANGE} \\
--tls-cert-file=${K8S_DIR}/pki/server.pem \\
--tls-cipher-suites=${CIPHER_TLS} \\
--tls-private-key-file=${K8S_DIR}/pki/server-key.pem \\
...
### 1、静态pod实现apiserver高可用

需要构建镜像，具体详细见`使用confd与nginx实现master节点高可用`,需要先部署kubelet
...
### 2、haproxy+keepalived实现apiserver高可用

#### 1 各master节点安装haproxy keepalived

```bash
for m in m{1..3};do ssh root@$m yum -y install haproxy keepalived;done
```
...
--bind-address=$(hostname -I|awk '{print $1}') \\
--alsologtostderr=true \\
--authentication-kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig \\
--authorization-kubeconfig=${K8S_DIR}/kube-scheduler.kubeconfig \\
--client-ca-file=${K8S_DIR}/pki/ca.pem \\
--kube-api-burst=150 \\
--kube-api-qps=100 \\
...
--requestheader-group-headers=${GROUP_HEADERS} \\
--requestheader-username-headers=${USERNAME_HEADERS} \\
--tls-cert-file=${K8S_DIR}/pki/kube-scheduler.pem \\
--tls-cipher-suites=${TLS_CIPHER} \\
--tls-private-key-file=${K8S_DIR}/pki/kube-scheduler-key.pem \\
--v=2
LimitNOFILE=65535
Restart=on-failure
[Install]
...
--cluster-signing-cert-file=${K8S_DIR}/pki/ca.pem \\
--cluster-signing-key-file=${K8S_DIR}/pki/ca-key.pem  \\
...
--flex-volume-plugin-dir=${K8S_DIR}/volume \\
...
--requestheader-group-headers=${GROUP_HEADERS} \\
--requestheader-username-headers=${USERNAME_HEADERS} \\
--root-ca-file=${K8S_DIR}/pki/ca.pem \\
--service-account-private-key-file=${K8S_DIR}/pki/ca-key.pem \\
--service-cluster-ip-range=${SERVICE_CIDR} \\
--terminated-pod-gc-threshold=50 \\
--tls-cert-file=${K8S_DIR}/pki/kube-controller-manager.pem \\
--tls-cipher-suites=${TLS_CIPHER} \\
--tls-private-key-file=${K8S_DIR}/pki/kube-controller-manager-key.pem \\
...
```bash
curl -s $(hostname -I|awk '{print $1}'):10252/metrics|tail -5
```
...
--healthz-port=10248 \\
--alsologtostderr=true \\
--bootstrap-kubeconfig=${K8S_DIR}/bootstrap-kubelet.kubeconfig \\
--cert-dir=${K8S_DIR}/pki \\
--cni-bin-dir=${CNI_DIR}/bin \\
--cni-conf-dir=${CNI_DIR}/net.d \\
--config=${K8S_DIR}/kubelet.yaml \\
...
--volume-plugin-dir=${K8S_DIR}/plugins \\
--v=2 
Restart=always
RestartSec=10s
[Install]
...
authentication:
  x509:
    clientCAFile: "${K8S_DIR}/pki/ca.pem"
  webhook:
    enabled: true
    cacheTTL: 2m0s
  anonymous:
...
```bash
curl -s $(hostname -I|awk '{print $1}'):10255/metrics|tail -5
```
...
```bash
curl -s $(hostname -I|awk '{print $1}'):10249/metrics|tail -5
```

#### 4 node节点确认使用IPVS模式

```bash
curl $(hostname -I|awk '{print $1}'):10249/proxyMode
```
...
--ca-file=${K8S_DIR}/pki/ca.pem \
--cert-file=${K8S_DIR}/pki/flanneld.pem \
--key-file=${K8S_DIR}/pki/flanneld-key.pem \
...
ExecStart=${FLANNEL_DIR}/bin/flanneld \\
  -etcd-cafile=${K8S_DIR}/pki/ca.pem \\
  -etcd-certfile=${K8S_DIR}/pki/flanneld.pem \\
  -etcd-keyfile=${K8S_DIR}/pki/flanneld-key.pem \\
...
```yaml
cat<< EOF >${CNI_DIR}/net.d/10-flannel.conflist
{
  "name": "cbr0",
  "cniVersion": "0.3.1",
  "plugins": [
...
FLANNEL_SUBNET=172.16.$(hostname -I|awk -F'[. ]+' '{print $(NF-1)}').1/24
FLANNEL_MTU=1450
FLANNEL_IPMASQ=true
EOF
```
...
--key-file=${K8S_DIR}/pki/flanneld-key.pem \
--endpoints="https://$(hostname -I|awk '{print $1}'):2379"  \
get /kubernetes/network/subnets/172.16.$(hostname -I|awk -F'[. ]+' '{print $(NF-1)}').0-24
```
...
不想master被调度时，需要进行相应的污点配置

```bash
[root@m1 k8s]# kubectl get node
NAME   STATUS    ROLES    AGE   VERSION
m1   Ready    <none>   23h   v1.18.18
m2   Ready    <none>   23h   v1.18.18
m3   Ready    <none>   23h   v1.18.18
n1   Ready    <none>   23h   v1.18.18
n2   Ready    <none>   23h   v1.18.18
n3   Ready    <none>   23h   v1.18.18
```
...
[root@m1 k8s]# kubectl get node
NAME    STATUS   ROLES    AGE   VERSION
m1   Ready    master   23h   v1.18.18
m2   Ready    master   23h   v1.18.18
m3   Ready    master   23h   v1.18.18
n1   Ready     node    23h   v1.18.18
n2   Ready     node    23h   v1.18.18
n3   Ready     node    23h   v1.18.18
```
...
```bash
[root@m1 ~]# kubectl get pod,svc,ep
NAME                        READY   STATUS    RESTARTS   AGE
pod/test-84b9fcd77d-678fx   1/1     Running   0          42s
pod/test-84b9fcd77d-6g5tz   1/1     Running   0          42s
pod/test-84b9fcd77d-ndtp7   1/1     Running   0          42s

NAME                 TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)   AGE
service/kubernetes   ClusterIP   10.96.0.1       <none>        443/TCP   14m
service/test         ClusterIP   10.96.232.171   <none>        80/TCP    42s

NAME                   ENDPOINTS                                         AGE
endpoints/kubernetes   10.0.0.31:6443,10.0.0.32:6443,10.0.0.33:6443   14m
endpoints/test         172.16.180.2:80,172.16.181.3:80,172.16.182.3:80   42s
```

访问应用

```bash
[root@m1 ~]# curl 10.96.232.171
Hello MyApp | Version: v2 | <a href="hostname.html">Pod Name</a>

[root@m1 ~]# curl 172.16.180.2
Hello MyApp | Version: v2 | <a href="hostname.html">Pod Name</a>
```
...
WORK_DIR=/apps
CNI_DIR=/apps/cni
ETCD_DIR=/apps/flannel
FLANNEL_DIR=/apps/etcd
K8S_DIR=/apps/kubernetes
ETCD_DATA_DIR=/apps/etcd/data
LOG_DIR=/apps/kubernetes/logs
SYSTEM_SERVICE_DIR=/usr/lib/systemd/system
...
#### 2  安装containerd或者docker

#### 3 创建目录

```bash
for n in n{1..3};do ssh $n "mkdir -p ${K8S_DIR}/{pki,manifests,plugins,bin} /root/.kube ${LOG_DIR} ${CNI_DIR}/{net.d,bin}  ${FLANNEL_DIR}/bin";done
```

#### 4 分发node所需软件

```bash
for n in n{1..3};do scp -r ${K8S_DIR}/bin/{kubelet,kube-proxy} $n:${K8S_DIR}/bin;done
for n in n{1..3};do scp -r ${FLANNEL_DIR} $n:${WORK_DIR};done
for n in n{1..3};do scp -r ${CNI_DIR}/bin $n:${CNI_DIR};done
```

#### 5 分发配置证书

```bash
for n in n{1..3};do scp ${K8S_DIR}/pki/{flanneld.pem,flanneld-key.pem,ca.pem} $n:${K8S_DIR}/pki;done
for n in n{1..3};do scp -r ${CNI_DIR}/net.d $n:${CNI_DIR};done
for n in n{1..3};do scp ${K8S_DIR}/{kube-proxy.kubeconfig,bootstrap-kubelet.kubeconfig,kubelet.yaml} $n:${K8S_DIR};done
for n in n{1..3};do scp ${SYSTEM_SERVICE_DIR}/flanneld.service $n:${SYSTEM_SERVICE_DIR};done
```
...
cat << EOF >/run/flannel/subnet.env
FLANNEL_NETWORK=172.16.0.0/16
FLANNEL_SUBNET=172.16.$(hostname -I|awk '{print $1}'|awk -F'[.]' '{print $NF}').1/24
FLANNEL_MTU=1450
FLANNEL_IPMASQ=true
EOF
```
...
```bash
for n in n{1..3};do ssh $n "systemctl enable  flanneld.service";done
