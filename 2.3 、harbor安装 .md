...
```
mkdir -p /data/software/ssl/
cd /data/software/ssl/
...
```
cat > habor.wzxmt.com-csr.json << EOF
{"CN":"habor.wzxmt.com","key":{"algo": "rsa","size":2048},"names":[{"C":"CN","L":"BeiJing","ST":"BeiJing","O":"kubernetes","OU":"k8s"}]}
EOF
cfssl gencert -ca=ca.pem -ca-key=ca-key.pem -config=ca-config.json -profile=kubernetes habor.wzxmt.com-csr.json | cfssljson -bare habor.wzxmt.com
```
...
```bash
tar xvf harbor-offline-installer-v2.0.0.tgz -C /usr/local/src/
...
https:
  port: 443
  certificate: /data/software/ssl/habor.wzxmt.com.pem
  private_key: /data/software/ssl/habor.wzxmt.com-key.pem
