...
upstream default_backend_traefik {
    server 10.0.0.41    weight=5 max_fails=3 fail_timeout=10s;
    server 10.0.0.42    weight=5 max_fails=3 fail_timeout=10s;
    server 10.0.0.43    weight=5 max_fails=3 fail_timeout=10s;
}
server {
    server_name *.wzxmt.com;
    access_log /var/log/nginx/http_access_log;
    error_log  /var/log/nginx/http_error_log;
    location / {
...
upstream default_backend_traefiks {
    server 10.0.0.41:443    max_fails=3 fail_timeout=10s;
    server 10.0.0.42:443    max_fails=3 fail_timeout=10s;
    server 10.0.0.43:443    max_fails=3 fail_timeout=10s;
}
server {
    listen       443 ssl;
    server_name  *.wzxmt.com;
    ...
    access_log /var/log/nginx/https_access_log;
    error_log  /var/log/nginx/https_error_log;
