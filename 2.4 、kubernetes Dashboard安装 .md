...
```bash
kubectl config set-cluster kubernetes --server=${KUBE_APISERVER} --kubeconfig=/root/dashbord.conf

kubectl config set-credentials dashboard-admin --token=$DASH_TOCKEN --kubeconfig=/root/dashbord.conf

kubectl config set-context dashboard-admin@kubernetes --cluster=kubernetes --user=dashboard-admin --kubeconfig=/root/dashbord.conf

kubectl config use-context dashboard-admin@kubernetes --kubeconfig=/root/dashbord.conf
```
...
```yaml
cat << 'EOF' >dashboard-ingressroute.yaml
