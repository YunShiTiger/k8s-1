...
注意不同版本的crd有可能不一样，详细见[https://doc.traefik.io/traefik/user-guides/crd-acme/](https://doc.traefik.io/traefik/user-guides/crd-acme/)
...
spec:
  entryPoints:
    - web
  routes:
  - match: Host(`test.wzxmt.com`) && PathPrefix(`/notls`)
    kind: Rule
    services:
    - name: whoami
      port: 80
...
spec:
  entryPoints:
    - websecure
  routes:
  - match: Host(`test.wzxmt.com`) && PathPrefix(`/tls`)
    kind: Rule
    services:
    - name: whoami
      port: 80
...
```bash
[root@supper traefik]# curl http://test.wzxmt.com/notls
Hostname: whoami-5db58df676-6flbm
IP: 127.0.0.1
IP: 172.16.40.160
RemoteAddr: 10.0.0.43:48100
GET /notls HTTP/1.1
Host: test.wzxmt.com
...
X-Forwarded-Host: test.wzxmt.com
X-Forwarded-Port: 80
X-Forwarded-Proto: http
X-Forwarded-Server: n3
X-Real-Ip: 10.0.0.11
```
...
```bash
[root@supper traefik]# curl -k https://test.wzxmt.com/tls
Hostname: whoami-5db58df676-6flbm
IP: 127.0.0.1
IP: 172.16.40.160
RemoteAddr: 10.0.0.43:48100
